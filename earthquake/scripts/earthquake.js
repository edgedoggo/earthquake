...
function renderQuickMenu(resolve) {
  ...
  form.querySelector("[data-earthquake-browse-sound]")?.addEventListener("click", () => {
    openSoundPicker(form.elements.soundPath);
  });
...
function openSoundPicker(input) {
  const FilePickerClass = globalThis.FilePicker ?? globalThis.foundry?.applications?.apps?.FilePicker;
  if (!FilePickerClass) {
    ui.notifications?.warn("Earthquake: Foundry File Picker is not available.");
    return;
  }
